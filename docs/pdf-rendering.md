...
No localhost. `sandbox=false`. Preview: `tmp/wedding-brief-v2/brief-preview.html` via `npm run test:wedding-brief`.

## Provider modules

| Concern | Module |
|---------|--------|
| HTML→PDF (brief) | Edge `pdf-render` via client `pdfShiftEdgeRenderer.ts` / `renderProductionPdf.ts` |
| DOCX→PDF (contract) | `supabase/functions/contract-docx-to-pdf/cloudmersiveConvert.ts` (canonical) + client `contractPdfAdapter.ts` |
| Provider diagnostics (optional) | `npm run pdf:pdfshift-poc` · `npm run pdf:cloudmersive-docx-poc` |
...
| `CLOUDMERSIVE_POC_LIVE` | POC script | Diagnostics only; CI must omit |
...
4. Expired Trial: PDF blocked (PRO)
5. Confirm brief still uses PDFShift
