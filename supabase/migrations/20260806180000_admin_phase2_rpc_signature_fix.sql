-- =============================================================================
-- Admin Phase 2 corrective migration — RPC signature / catalog fix
-- Fixes 42883 / PostgREST 404 for admin_* aggregates (esp. registration_series).
-- Does not edit the already-applied 20260806140000 migration.
-- =============================================================================

-- Drop possible mismatched overloads so CREATE binds the frontend contracts.
drop function if exists public.admin_get_registration_series(integer);
drop function if exists public.admin_get_registration_series();
drop function if exists public.admin_get_overview_metrics(text);
drop function if exists public.admin_get_overview_metrics();
drop function if exists public.admin_get_product_usage();
drop function if exists public.admin_get_activation_funnel();
drop function if exists public.admin_get_attention_items();
drop function if exists public.admin_list_users(integer, integer, text, text);
drop function if exists public.admin_get_user_summary(uuid);
drop function if exists public.admin_get_integration_health();
drop function if exists public.admin_get_system_health();
drop function if exists public.admin_list_audit(integer, integer, text);
drop function if exists public.admin_get_email_metrics();
...
declare
  _uid uuid;
  day_count integer := greatest(1, least(coalesce(p_days, 30), 90));
  series jsonb;
  range_start_local date;
  range_end_local date;
begin
  _uid := public.assert_admin_owner_aal2();

  range_end_local := (timezone('Europe/Warsaw', now()))::date;
  range_start_local := range_end_local - (day_count - 1);

  with day_spine as (
    select generate_series(range_start_local, range_end_local, interval '1 day')::date as day
  ),
  counts as (
    select (timezone('Europe/Warsaw', u.created_at))::date as day, count(*)::bigint as c
    from auth.users u
    where u.created_at >= (range_start_local::timestamp at time zone 'Europe/Warsaw')
      and u.created_at < ((range_end_local + 1)::timestamp at time zone 'Europe/Warsaw')
    group by 1
  )
...
-- Explicit privilege posture for browser-facing RPCs
revoke all on function public.admin_get_overview_metrics(text) from public, anon;
revoke all on function public.admin_get_registration_series(integer) from public, anon;
revoke all on function public.admin_get_product_usage() from public, anon;
revoke all on function public.admin_get_activation_funnel() from public, anon;
revoke all on function public.admin_get_attention_items() from public, anon;
...
revoke all on function public.admin_get_email_metrics() from public, anon;
...
revoke all on function public.admin_list_audit(integer, integer, text) from public, anon;
...
-- Force PostgREST to reload function catalog after signature repair
