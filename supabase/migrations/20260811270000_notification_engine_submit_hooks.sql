-- Wire questionnaire submit RPCs to notification engine.
-- Replaces direct notifications inserts with exactly-once enqueue helpers.
...
begin
  ...
  -- Notification engine V1 (in-app + optional email). Idempotent on instance id.
...
begin
  ...
  -- Notification engine V1 (in-app + optional email). Key includes submitted_at for resubmits.
  perform public.notify_prewedding_questionnaire_completed(
    v_qid,
    v_submitted_at,
    v_was_submitted
  );
