...

{
  ...
  assertIncludes(header, "confirmText !== 'USUŃ'", 'typed USUŃ required')
  assertIncludes(header, 'Wpisz USUŃ, aby potwierdzić', 'USUŃ confirm label')
  assertIncludes(header, 'Usuń na zawsze', 'destructive confirm CTA')
  assertNotIncludes(header, 'window.confirm', 'no native confirm for session delete')
  assertIncludes(header, 'requirePro', 'PRO gate on delete')
  assertIncludes(header, 'Modal', 'delete uses Modal')
