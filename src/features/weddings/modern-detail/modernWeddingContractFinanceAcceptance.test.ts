...
run('H.3.2: one commercial summary row, Wartość umowy, no Termin minął', () => {
  ...
  assert(modern.includes('docActionPrimary'), 'Podgląd emphasized among doc actions')
  assert(modern.includes('workflowAction'), 'lifecycle actions are secondary')
  ...
  assert(modern.includes('discloseTrail'), 'info row right cluster')
