...
assert(ui.includes('root.scrollTo'), 'scroll via layout scroll root')
assert(!ui.includes('btn?.scrollIntoView'), 'no sticky-chip scrollIntoView')
assert(!ui.includes('href="#"'), 'no hash year anchors')
assert(!ui.includes('window.scrollTo'), 'no window.scrollTo')
