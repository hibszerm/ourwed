...
/* —— Features grid —— */
{
  ...
  assertIncludes(features, 'HEADER_OFFSET_DESKTOP', 'desktop header clock token frozen')
  assertIncludes(features, "['start 0.92', 'start 0.5']", 'desktop header clock values frozen')
...

{
  ...
  assertIncludes(story, 'Layers3', 'compact History uses Layers3 (3H.1)')
  assertIncludes(story, 'SecurityLockGraphic', 'Security lock remains in Security DOM')
  assertIncludes(story, 'data-compact-security-section', 'compact Security chapter (3H)')
  assertIncludes(storyCss, 'animation-timeline: view()', 'CSS view-timeline entry (3H)')
  assertNotIncludes(story, 'whileInView', 'no Framer whileInView scrub (3H)')
  ...
  /* —— Mobile owns phone→lock on desktop; compact 3H native exit —— */
  assertIncludes(
    mobile,
    "data-mobile-post-brief-owner={isCompactViewport ? 'false' : 'true'}",
    'desktop owns post-Brief morph; compact does not',
  )
  assertIncludes(
    mobile,
    "data-mobile-phone-lock-owner={isCompactViewport ? 'false' : 'true'}",
    'desktop owns phone→lock; compact native exit',
  )
  assertIncludes(mobile, 'postBriefProgress', 'Mobile postBriefProgress MotionValue')
  assertIncludes(mobile, 'postBriefRunwaySvh', 'post-Brief tail runway helper')
  assertIncludes(mobile, 'postBriefShrinkScaleAt', 'LINEAR shrink from Mobile timeline (desktop)')
  ...
  assertIncludes(mobile, 'data-security-copy=""', 'desktop security copy in Mobile sticky')
  ...
  assertIncludes(story, 'LV2_SECURITY_COPY', 'compact Security copy in document-flow story (3H)')
  assertIncludes(story, 'SecurityLockGraphic', 'Security lock graphic in Security section')
  assertIncludes(story, 'Layers3', 'compact History Layers3 icon (3H.1)')
  ...
  assertIncludes(mobile, 'native-exit', 'compact 3H native phone exit (no morph theater)')
  assertNotIncludes(mobile, 'compactIntro', 'compact sticky History intro removed (3H)')
  ...
  assertIncludes(mobile, 'data-studio-lock={isCompactViewport ? undefined : \'\'}', 'desktop morph lock marker')
  ...
  assertIncludes(mobileCss, '--mobile-track-season-import-svh: 0', 'compact clears import sticky runway')
  assertEq(MOBILE_TRACK_STUDIO_HISTORY_SVH, 145, 'desktop studio history 145svh')
  assertIncludes(mobile, 'isCompactViewport ? 0', 'compact studio/postBrief runway zeroed (3H)')
  ...
  assertEq(studioCompactLockFadeAt(1), 1, 'compact sticky lock fade unused')
  {
    ...
    assertIncludes(historyReveal, "data-studio-history-compact={compactIntro ? 'intro' : 'false'}", 'desktop history marker prop retained')
    assertNotIncludes(historyReveal, 'yearsPeek', 'no sticky 2026 peek (3H)')
...

{
  ...
  assertIncludes(founderFlow, '<FounderQualification localReveal={compact}', 'compact qualification local reveal')
  assertIncludes(founderFlow, 'compactLayout={compact}', 'compact qualification layout flag')
  ...
  assertIncludes(founderContent, 'data-founder-compact-editorial', 'compact editorial architecture marker')
  assertIncludes(founderContent, "data-founder-story-chapter=\"1\"", 'story chapter 1 marker')
  assertIncludes(founderContent, "data-founder-story-chapter=\"2\"", 'story chapter 2 marker')
  assertIncludes(founderContent, 'data-founder-manifesto', 'manifesto chapter marker')
  assertIncludes(founderContent, 'whileInView', 'compact local whileInView reveals')
  assertIncludes(founderContent, '[0.22, 1, 0.36, 1]', 'compact reveal ease matches Features/History')
  assertIncludes(founderContent, "margin: '0px 0px -25% 0px'", 'portrait reveal uses lower viewport band')
  assertIncludes(founderContent, 'scale: 0.99', 'portrait soft scale entry')
  assertIncludes(founderContent, "data-founder-profile={compact ? 'horizontal' : 'stacked'}", 'compact horizontal profile marker')
  assertIncludes(founderContent, 'x: 10', 'compact identity trails with x reveal')
  assertNotIncludes(founderContent, 'position: sticky', 'no sticky inside Founder content')
  assertNotIncludes(founderContent, 'useScroll', 'no parent scroll progress scrub')
  assertIncludes(founderCss, 'aspect-ratio: 4 / 5', 'compact portrait card 4/5')
  assertIncludes(founderCss, 'flex-direction: row', 'compact profile is horizontal')
  assertIncludes(founderCss, 'clamp(8.5rem, 37vw, 9.875rem)', 'compact portrait width band')
  assertIncludes(founderCss, "storyChapter[data-founder-story-chapter='1']", 'compact chapter 1 spacing')
  assertIncludes(founderCss, "storyChapter[data-founder-story-chapter='2']", 'compact chapter 2 beat')
  assertIncludes(founderCss, '--founder-gap-23: clamp(4.375rem, 9vw, 5.25rem)', 'tighter story→manifesto gap')
  assertIncludes(founderCss, '--founder-bottom: 0', 'compact canvas bottom does not double-space before qualification')
  assertIncludes(founderCss, 'grid-template-columns: 31.25rem minmax(0, 47.5rem)', 'desktop portrait-left story-right preserved')
  assertIncludes(founderCss, 'aspect-ratio: 3 / 4', 'desktop portrait 3/4 preserved')
  assertIncludes(founderCss, 'min-width: 1101px', 'desktop compression queries stay desktop-only')
  assertNotIncludes(founderCss, 'position: sticky', 'Founder CSS has no sticky theater')
...

{
  ...
  assertIncludes(qual, 'Content creator', 'audience Content creator')
  assertNotIncludes(qual, 'Twórcy ślubni', 'audience Twórcy ślubni removed')
  ...
  /* Qualification: centered Tier-A, chapter marker; desktop static, compact local reveal */
  ...
  assertIncludes(qualUi, "data-qualification-motion={reveal ? 'local' : 'none'}", 'compact local / desktop none motion')
  assertIncludes(qualUi, 'localReveal', 'qualification accepts localReveal')
  assertIncludes(qualUi, 'whileInView', 'compact signals use whileInView')
  assertIncludes(qualUi, "data-audience-layout={compactLayout ? 'grid-2x2' : 'row'}", 'compact audience 2×2 marker')
  assertIncludes(qualUi, 'compactLayout', 'qualification accepts compactLayout')
  assertIncludes(qualUi, 'LV2_QUALIFICATION_SIGNALS.map', 'canonical numbered signals map')
  assertIncludes(qualUi, 'LV2_QUALIFICATION_AUDIENCE.labels.map', 'canonical audience labels')
  assertNotIncludes(qualUi, 'useConversionReveal', 'qualification has no conversion reveal hook')
  ...
  assertIncludes(qualCss, 'grid-template-columns: repeat(2, minmax(0, 1fr))', 'desktop signal 2×2 + compact audience grid')
  assertIncludes(qualCss, '@media (max-width: 1100px)', 'compact qualification editorial media')
  assertIncludes(qualCss, 'min-height: 4.75rem', 'compact audience cell min height')
  assertIncludes(qualCss, 'padding: clamp(1.5rem, 4vw, 1.75rem) 0', 'compact signal row padding')
  ...
  assertIncludes(pricingCss, 'margin: clamp(3rem, 4.5vw, 4.25rem) auto 0', 'desktop intro→trial margin preserved')
  assertIncludes(pricingCss, 'margin-top: clamp(2.5rem, 3.7vw, 3.5rem)', 'compact intro→trial gap tightened ~15–20%')
  assertIncludes(pricingCss, 'padding: 1.05rem 1.15rem', 'compact trial card slightly lighter padding')
  ...
  assertIncludes(footer, 'LEGAL_ROUTES.terms', 'footer regulamin via LEGAL_ROUTES')
  assertIncludes(footer, 'LEGAL_ROUTES.privacy', 'footer polityka via LEGAL_ROUTES')
  assertIncludes(footer, 'LEGAL_ROUTES.dpa', 'footer powierzenie via LEGAL_ROUTES')
  assertIncludes(footer, 'href={LEGAL_ROUTES.terms}', 'footer regulamin href')
  assertIncludes(footer, 'href={LEGAL_ROUTES.privacy}', 'footer polityka href')
  assertIncludes(footer, 'href={LEGAL_ROUTES.dpa}', 'footer powierzenie href')
