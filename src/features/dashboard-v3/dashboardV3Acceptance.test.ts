...

{
  ...
  assert(today.includes('FloatingPortal'), 'horizon menu portals above sibling cards')
  assert(today.includes('forceAnchored: true'), 'horizon menu stays anchored on mobile')
  assert(today.includes('data-testid="dashboard-v3-horizon-menu"'), 'horizon menu test id')
  assert(!today.includes('horizonMenuAbove'), 'placement handled by FloatingPortal')
  ...
  assert(!todayCss.includes('.horizonMenu {\n  position: absolute'), 'horizon menu not absolutely clipped in-card')
  assert(!todayCss.includes('z-index: 8'), 'no local stacking fight with next card')
  assert(!todayCss.includes('overflow: auto'), 'today panel is not a scroll widget')
