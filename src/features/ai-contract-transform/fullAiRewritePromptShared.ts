/**
 * Shared Full-AI rewrite prompt/schema — MUST stay in sync with
 * supabase/functions/ai-contract-full-rewrite/prompt.ts
 *
 * Node/CG2 harness imports this file. Edge keeps its Deno copy for deploy isolation.
 */
...
export function resolveModelFromEnv(getEnv: (name: string) => string | undefined): string {
  return (
    getEnv('OPENAI_CONTRACT_TRANSFORM_MODEL')?.trim() ||
    getEnv('OPENAI_CONTRACT_MODEL')?.trim() ||
    'gpt-4.1-mini'
  )
}
...
- When requiredReplacements lists wedding.*Location with sourceBlockIds, those blocks/cells are grounded event-location fields (preparations / ceremony / reception). Put the matching canonical location into EACH listed block. Do not swap ceremony/reception/preparation. Do not invent a venue when targetRenderedValues is "—" — neutralize the field only. Preserve table value cells as value-only text (do not overwrite row labels).
- When rewriting a block that contains BOTH provider identity (NIP/REGON/studio) AND customer identity in the same paragraph: preserve the provider half EXACTLY (name, address, NIP, REGON, email, phone, role closing such as "zwaną dalej Fotografem"). Only rewrite the customer half (names, address, phone, email, "zwaną dalej Klient…"). Never label the customer as Fotograf/Wykonawca/Usługodawca.
...
- DATA TRANSFORMATION only: do NOT improve wording, normalize legal terminology, fix grammar/style outside blocks that require canonical fact changes, modernize the contract, or paraphrase unrelated clauses. Do NOT change Photographer/Filmmaker/provider-role language when no canonical fact in that block needs updating.
